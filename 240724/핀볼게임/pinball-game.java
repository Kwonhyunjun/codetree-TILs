...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        for(int i=0; i<N; i++){
            // 위(아래쪽 방향)
            answer = Math.max(answer, solve(-1, i, 1));
            // 왼쪽(오른쪽 방향)
            answer = Math.max(answer, solve(i, -1, 3));
            // 오른쪽(왼쪽 방향)
            answer = Math.max(answer, solve(i, N, 2)); 
            // 아래쪽
            answer = Math.max(answer,  solve(N, i, 0)); 
        }
    ...
    static int turn(int type, int d){
        int res = 0; 
        // String prev = (d == 0) ? "상" : (d ==1)? "하" : (d==2)? "좌" : "우"; 
        if(type == 1){
            switch(d){
                case 0 :
                    res = 3; 
        ...
        // String next = (res == 0) ? "상" : (res ==1)? "하" : (res==2)? "좌" : "우"; 
